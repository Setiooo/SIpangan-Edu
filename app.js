...
const field = (label, ph, { icon = '', type = 'text', help = '', error = '' } = {}) => `
  <label class="block">
    ...
    ${error ? `<span class="mt-1 text-[12px] text-danger flex items-center gap-1">${ic('circle-alert','w-3 h-3')}${error}</span>` : help ? `<span class="mt-1 text-[12px] text-ink-soft">${help}</span>` : ''}
  </label>`;
...
/* 1. Splash */
S['splash'] = () => `
  <div class="absolute inset-0 flex flex-col items-center justify-center bg-primary text-white px-8 text-center">
    ...
    <div class="mt-10 w-full max-w-[260px]"><button onclick="go('login')" class="tap w-full h-12 rounded-btn bg-white text-primary font-semibold shadow-soft flex items-center justify-center gap-2">${ic('log-in','w-4 h-4')}Masuk</button></div>
    <button onclick="go('onboarding-1')" class="tap absolute bottom-10 text-xs text-white/70 underline">Lihat intro</button>
  </div>`;
  ...
  <div class="flex-1 px-5 py-4 space-y-4">
    <div><h3 class="font-display font-semibold mb-2">Aksi Cepat</h3>
      <div class="grid grid-cols-4 gap-2 text-center">
        ${[['Jadwal','calendar-days','distribution-schedule'],['Sekolah','school','school-list'],['Statistik','chart-column','statistics'],['Laporan','clipboard-list','admin-reports']].map(([l,i,t])=>`<button onclick="go('${t}')" class="tap flex flex-col items-center gap-1.5"><span class="w-12 h-12 rounded-2xl bg-primary-light text-primary grid place-items-center">${ic(i)}</span><span class="text-[11px] text-ink-soft">${l}</span></button>`).join('')}
      </div></div>
...
S['qr-scanner'] = () => screenWrap(`
  <div class="flex-1 flex flex-col bg-slate-900 text-white relative">
    ...
    <div class="p-6"><button onclick="go('verification-success')" class="tap w-full h-12 rounded-btn bg-accent text-slate-900 font-bold flex items-center justify-center gap-2">${ic('circle-check','w-5 h-5')}Simulasi Scan Berhasil</button></div>
...
S['distribution-report'] = () => screenWrap(`
  ...
  <div class="flex-1 px-5 py-4 space-y-4">
    <div class="grid grid-cols-2 gap-3">${[['Jatah Dijadwalkan','240','package','text-primary'],['Diterima Sekolah','240','circle-check','text-secondary'],['Selisih','0','equal','text-ink-soft'],['Akurasi','100%','target','text-accent-dark']].map(([l,v,i,c])=>card(`${ic(i,'w-5 h-5 '+c)}<p class="font-display font-bold text-2xl mt-2">${v}</p><p class="text-[12px] text-ink-soft">${l}</p>`)).join('')}</div>
...
S['report-form'] = () => screenWrap(`
  ...
  <div class="flex-1 px-5 py-4 space-y-4">
    <div><p class="text-[13px] font-semibold mb-2">Jenis Laporan</p><div class="grid grid-cols-2 gap-2">${[['Jumlah Kurang','circle-minus'],['Terlambat','alarm-clock'],['Tidak Datang','truck',],['Kualitas Makanan','utensils-crossed']].map(([t,i],idx)=>`<button class="tap flex items-center gap-2 p-3 rounded-btn border ${idx===0?'border-primary bg-primary-light text-primary':'border-line bg-white text-ink-soft'}">${ic(i,'w-4 h-4')}<span class="text-[12px] font-semibold">${t}</span></button>`).join('')}</div></div>
...
S['statistics'] = () => screenWrap(`
  ...
  <div class="flex-1 px-5 py-4 space-y-4">
    <div class="grid grid-cols-2 gap-3">${[['Total Porsi','28.4k','package','text-primary'],['Sekolah Aktif','128','school','text-secondary'],['Akurasi Jatah','98%','target','text-accent-dark'],['Laporan Selesai','92%','circle-check','text-primary']].map(([l,v,i,c])=>card(`${ic(i,'w-5 h-5 '+c)}<p class="font-display font-bold text-2xl mt-1">${v}</p><p class="text-[12px] text-ink-soft">${l}</p>`)).join('')}</div>
...
S['petugas-home'] = () => screenWrap(`
  <div class="px-5 pt-3 pb-5 bg-primary text-white rounded-b-3xl">
    ...
    <div class="grid grid-cols-3 gap-2 mt-4">${[['Tugas','4','clipboard-list'],['Selesai','1','circle-check'],['Porsi','1.150','package']].map(([l,v,i])=>`<div class="bg-white/15 rounded-2xl p-3 text-center">${ic(i,'w-4 h-4 mx-auto mb-1')}<p class="font-display font-bold text-lg">${v}</p><p class="text-[10px] text-white/80">${l}</p></div>`).join('')}</div>
  </div>
  <div class="flex-1 px-5 py-4 space-y-4">
...
/* ===================== NAVIGASI PROTOTYPE ===================== */
let current = 'splash';
function go(id){ if(!S[id])return; current=id; const [no,title,mod]=META[id]; document.getElementById('screen').innerHTML=`<div class="screen-enter h-full">${S[id]()}</div>`; document.getElementById('stageTitle').textContent=title; document.getElementById('stageMeta').textContent=(typeof no==='number'?`Layar ${no} · `:'Pendukung · ')+mod; buildIndex(); if(window.lucide)lucide.createIcons(); document.getElementById('screen').scrollTop=0; }
